...
On September 8, 2026, the user explicitly requested syncing this website to https://github.com/TrustAIRLab/PersonaHate. The designated `webpage` branch has independent history and contains the website source plus a prebuilt `docs/` site; the research `main` branch is preserved. The repository was private at preparation time. This request authorizes uploading the website and its existing assets to that repository; it does not change repository visibility or enable public Pages hosting.

`npm run build:pages` uses Vinext static export and the `/PersonaHate/` base path. The `docs/.nojekyll` marker preserves access to `_next/` assets on GitHub Pages. Local hosting metadata, dependencies, caches, and development logs are excluded from the branch.

Vinext 1.0.0-beta.5 omits the configured base path in its internal homepage prerender request, causing a 404 that it records as skipped/dynamic. The Pages preparation script handles this known case by rendering `/PersonaHate/` through the built production server, saving the complete HTML and React payload, and closing the temporary server. Dependency files are not patched.
