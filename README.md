...
This independent `webpage` branch contains the homepage source and its prebuilt GitHub Pages site. Research code stays on [`main`](https://github.com/TrustAIRLab/PersonaHate/tree/main). The branches have separate histories.

## Contents

- `app/page.tsx`: paper overview, original figures, dataset and resource descriptions.
...
```sh
git clone --branch webpage --single-branch https://github.com/TrustAIRLab/PersonaHate.git
cd PersonaHate
npm ci
npm run dev
```

The local preview uses the root path. `npm run build:pages` builds the static website for the `/PersonaHate/` GitHub Pages path and refreshes `docs/`. PDF, persona, figure, and JavaScript URLs use the same base path.
...
3. Select **webpage** and **/docs**, then save.

After Pages is enabled and its deployment succeeds, the default address is `https://trustairlab.github.io/PersonaHate/`. The repository must have a GitHub plan that supports Pages for its visibility. Enabling Pages is a separate step; pushing this branch does not enable it. See [GitHub's publishing-source instructions](https://docs.github.com/en/pages/getting-started-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site).

For later updates, edit the source, run `npm run build:pages`, and commit both the source changes and the refreshed `docs/` directory on `webpage`. Keep research code changes on `main`.
